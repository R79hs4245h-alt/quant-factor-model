...
class ExchangeTradedFundRecommender:
    ...
    def __init__(self):
        ...
        self.four_dim_engine = FourDimensionEngine() if _ADVANCED_AVAILABLE else None
    ...
    def calculate_scores(self, df: pd.DataFrame = None) -> pd.DataFrame:
        ...
        if self.four_dim_engine is not None:
            # 使用实时行情数据构造代理K线 (无历史K线时的降级方案)
            # 当有真实K线数据时,应通过 kline_fetcher 获取完整OHLC
            adv_scores = []
    ...
    def _calc_advanced_score_for_row(self, row: pd.Series) -> Dict:
        ...
        当仅有实时行情数据(无历史K线)时,使用降级方案:
          - 构造单日OHLC代理K线
          - 估值维度使用可用数据(若有PE/PB)
          - 资金维度使用成交额/换手率代理
          - 技术维度(拐点/主升浪)因K线不足自动降分

        当有完整历史K线时,四维引擎将完整执行所有检测。
        """
        ...
        result = self.four_dim_engine.analyze(
            ohlc=ohlc,
            # 估值数据(若有)
            pe=row.get('pe') if 'pe' in row.index else None,
            pb=row.get('pb') if 'pb' in row.index else None,
            # 资金维度: 用成交额代理大单
            large_order_net=float(row.get('amount_yuan', 0)) / 1e4 if pd.notna(row.get('amount_yuan', 0)) else None,
        )
    ...
    def _format_recommendation(self, fund: Dict, theme: str, rotation: Dict) -> Dict:
        ...
        # 信号融合: 四维共振信号优先级高于基础信号
        if adv_signal in ('强买入',):
            signal = '强买入'
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            'advanced_analysis': {
                ...
                'dimensions': ['估值相对合理', '触底反弹拐点', '主力资金流入', '主升浪启动'] if self.four_dim_engine else [],
