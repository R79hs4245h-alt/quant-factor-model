...
def run_portfolio_prediction():
    """
    v7.5: 投资组合预测引擎

    用10万启动资金,基于30日历史行情数据,
    在下跌风险低、上涨趋势明显、估值合理的前提下,
    预测未来2日盈利概率最高的投资组合。
    """
    print("=" * 70)
    print("  投资组合预测引擎 v7.5")
    print("  10万资金 | 30日历史数据 | 2日盈利概率预测")
    print("=" * 70)
    ...
    predictor = PortfolioPredictor(capital=100000)
    result = predictor.run()

    # 打印结果
    print(f"\n  报告日期: {result['report_date']}")
    print(f"  启动资金: {result['capital']:,.0f}元")
    print(f"  候选标的: {result['candidates_total']}只")
    print(f"  K线获取: {result['kline_fetched']}只")
    print(f"  筛选通过: {result['screen_passed']}只")
    print(f"  高概率候选: {result['high_prob_count']}只")

    # 市场分析
    ma = result.get('market_analysis', {})
    print(f"\n  --- 市场分析 ---")
    print(f"  市场状态: {ma.get('status', '未知')}")
    print(f"  上涨占比: {ma.get('breadth', 0):.1%} ({ma.get('up_count', 0)}/{ma.get('total_observed', 0)})")
    print(f"  平均涨跌: {ma.get('avg_change', 0):+.2%}")

    # 组合概况
    port = result.get('portfolio', {})
    print(f"\n  --- 投资组合 ---")
    print(f"  组合盈利概率: {port.get('portfolio_profit_prob', 0):.1%}")
    print(f"  预期2日收益: {port.get('portfolio_expected_return', 0):+.2%}")
    print(f"  组合风险指数: {port.get('portfolio_risk', 0):.3f} (越低越安全)")
    print(f"  投入资金: {port.get('total_invested', 0):,.0f}元")
    print(f"  现金保留: {port.get('cash_reserve', 0):,.0f}元")

    alloc = port.get('allocation_summary', {})
    print(f"  持仓数量: {alloc.get('n_holdings', 0)}只")
    print(f"  ETF占比: {alloc.get('etf_ratio', 0):.1%}")
    print(f"  股票占比: {alloc.get('stock_ratio', 0):.1%}")
    print(f"  最大持仓: {alloc.get('max_position', 0):.1%}")

    # 持仓明细
    holdings = port.get('portfolio', [])
    if holdings:
        print(f"\n  --- 持仓明细 ---")
        print(f"  {'代码':<8} {'名称':<12} {'类型':<5} {'主题':<8} {'价格':>8} {'股数':>6} {'金额':>10} {'权重':>6} {'盈利概率':>8} {'预期收益':>8}")
        print(f"  {'-'*100}")
        for h in holdings:
            print(f"  {h['code']:<8} {h['name'][:10]:<12} {h['asset_type']:<5} {h.get('theme','')[:6]:<8} "
                  f"{h['current_price']:>8.3f} {h['shares']:>6} {h['amount']:>10,.0f} "
                  f"{h['weight']:>5.1%} {h['profit_prob']:>7.1%} {h['expected_return']:>+7.2%}")

        print(f"\n  --- 主题配置 ---")
        for theme, pct in alloc.get('theme_allocation', {}).items():
            bar = '█' * int(pct * 30)
            print(f"  {theme:<10}: {bar} {pct:.1%}")

        print(f"\n  --- 止损止盈 ---")
        for h in holdings:
            print(f"  {h['code']} {h['name'][:8]}: 止损={h['stop_loss']:.3f}({(h['stop_loss']/h['current_price']-1)*100:+.1f}%) "
                  f"止盈={h['target_profit']:.3f}({(h['target_profit']/h['current_price']-1)*100:+.1f}%)")

    # 保存结果
    filepath = predictor.save_results(result)
    print(f"\n  结果已保存: {filepath}")
    print("=" * 70)
